...
export class MongoDb implements IMongoDBConfig {
  ...
  constructor () {
    ...
    this.dbUser = mongodb.dbUser || null
    this.dbPassword = mongodb.dbPassword || null
  }

  public connect () {
    let MONGO_URI = `mongodb://${this.dbHost}:${this.dbPort}/?authSource=${this.dbName}` // prettier-ignore

    let options = {}
    if (this.auth) {
      MONGO_URI = `mongodb://${this.dbUser}:${this.dbPassword}@${this.dbHost}:${this.dbPort}/?authSource=${this.dbName}`
      options = {
